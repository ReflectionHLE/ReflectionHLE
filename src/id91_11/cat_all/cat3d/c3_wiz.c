...
//
	if (gamestate.shotpower)
	{
		...
		do
		{
			BE_SDL_EGAUpdateGFXBufferScrToScr(dest+PAGE1START, source, 5);
			BE_SDL_EGAUpdateGFXBufferScrToScr(dest+PAGE2START, source, 5);
			BE_SDL_EGAUpdateGFXBufferScrToScr(dest+PAGE3START, source, 5);
			source += 5;
			dest += SCREENWIDTH;
		} while (--linesleft);
#if 0

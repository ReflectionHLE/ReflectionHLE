...
// REFKEEN: LocalTime has no use and TimeCount is updated differently.
// But make use of SoundUserHook for the intermission screen.
extern void (*SoundUserHook)(void);

static void TIME(void)
{
	if (!(count_time & 1) && SoundUserHook)
	{
//		++LocalTime;
//		++TimeCount;
//		count_time = 0;

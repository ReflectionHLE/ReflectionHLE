	...
	for (y=0;y<height;y++)
	{
		...
		for (x=0;x<width;x++)
		{
			...
			for (b=0;b<8;b++)
			{
				...
				//color = 0; // (CHOCO CAT) COMMENTED OUT (cause color is (re)set to another value now)

				// (CHOCO CAT) Originally only the low byte of
...
// NOTE (CHOCO CAT) This is useless on modern and with (somewhat) portable code,
...
//
		// (CHOCO CAT) Kind of backwards compatibility
		work->codeofs[src] = (code-&work->code[0]);
...
// (CHOCO CAT) Replacement for functions generated by BuildCompScale
void ExecuteCompScale(const t_compscale *compscale, id0_unsigned_t egaDestOff, id0_byte_t *srcPtr)
{
	const id0_byte_t *code = &compscale->code[0];
	while (*code == 0x8a)
	{
		//
		// mov al, [si+src] ;We fetch src from code
		//
		id0_byte_t srcVal = = srcPtr[*(code += 2)];
		++code;
		while (*code == 0x26)
		{
			//
			...
			BE_SDL_EGAAndGFXByte(egaDestOff+heightofs, srcVal, 15);
		}
	}
}


// NOTE (CHOCO CAT): Like BuildCompScale, we keep this code for compatibility
...
{
	// (CHOCO CAT) Use offsets into (planar) EGA memory instead
...
//
	...
	BE_SDL_EGAUpdateGFXBuffer(workEgaOff+offsetof(t_compshape, width), (uint8_t *)&width, sizeof(width), 1);
	//work->width = width;
...
//
	...
	for (x=firstline;x<=lastline;x++)
		for (y=0;y<64;y++)
			if (spotvis[y][x] != BACKGROUNDPIX)
				BE_SDL_EGAUpdateGFXByte((codeEgaOff++)-workEgaOff, spotvis[y][x], 1);
				//*code++ = spotvis[y][x];
...
//
	for (x=firstline;x<=lastline;x++)
	{
		...
		BE_SDL_EGAUpdateGFXBuffer(workEgaOff+offsetof(t_compshape, work->codeofs[x-firstline]), (uint8_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 1);
		...
		//
			...
			BE_SDL_EGAUpdateGFXBuffer(codeEgaOff, (uint8_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 1);
			codeEgaOff += 2;
...
void ScaleShape (id0_int_t xcenter, t_compshape id0_seg *compshape, id0_unsigned_t scale)
{
	...
	// Includes self-generated code executing...self-generated code
#if 0

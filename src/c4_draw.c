...
void near ScaleOneWall (id0_int_t xl, id0_int_t xh)
{
	...
	for (x=xl;x<=xh;x+=pixwidth)
	{
		...
		ExecuteCompScale(scaledirectory[height], egaDestOff, wallscalesourceptr, bitmasks1[x&7][pixwidth-1]);
		if (bitmasks2[x&7][pixwidth-1])
		{
			...
			++egaDestOff;
			// (CHOCO CAT) Replacement for functions generated by BuildCompScale
			ExecuteCompScale(scaledirectory[height], egaDestOff, wallscalesourceptr, bitmasks2[x&7][pixwidth-1]);
		}
#if 0
		//
...
void ClearScreen (void)
{
	...
	{
		// (CHOCO CAT) DIFFERENCE FROM VANILLA:
		...
		// skycolor always pointers to a member of sky_colors, where each pair has the exact same color repeated twice.
		//
	...
	for (int loopVar = CENTERY+1; loopVar; --loopVar)
	{
		// (CHOCO CAT) DIFFERENCE FROM VANILLA: Same as above but with bottomcolor

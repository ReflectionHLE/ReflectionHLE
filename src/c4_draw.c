...
void near ScaleOneWall (id0_int_t xl, id0_int_t xh)
{
	...
	for (x=xl;x<=xh;x+=pixwidth)
	{
		...
		*(((id0_unsigned_t *)&scaletablecall)+1) = (id0_unsigned_t)scaledirectory[height];
		...
		// TODO (CHOCO CAT) IMPLEMENT PROPERLY! (using bitmasks1 mask)
		//bitmasks1[x&7][pixwidth-1]
		scaletabelcall(); // scale the line of pixels
		//
		if (bitmasks2[x&7][pixwidth-1])
		{
			...
			//
			++egaDestOff;
			// TODO (CHOCO CAT) SIMILARLY IMPLEMENT but with bitmasks2
			scaletabelcall(); // scale the line of pixels
		}
#if 0
		//
